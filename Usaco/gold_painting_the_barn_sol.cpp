...
int main() {
	...
	vector<vector<int>> pref_leftovers(WIDTH, vector<int>(WIDTH));
	for (int r = 1; r < WIDTH; r++) {
		for (int c = 1; c < WIDTH; c++) {
			pref_leftovers[r][c] =
			    (pref_leftovers[r - 1][c] + pref_leftovers[r][c - 1] -
			     pref_leftovers[r - 1][c - 1] + leftovers[r][c]);
		}
	}

	// returns the sum of leftovers[from_r][from_c] to leftovers[to_r][to_c]
	auto rect_sum = [&](int from_r, int from_c, int to_r, int to_c) {
		return (pref_leftovers[to_r][to_c] - pref_leftovers[from_r-1][to_c] -
		        pref_leftovers[to_r][from_c-1] + pref_leftovers[from_r-1][from_c-1]);
	};
